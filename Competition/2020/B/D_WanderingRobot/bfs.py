...
def solve(w, h, l, u, r, d) -> float:
    ...
    grid[0][0] = 1
    ishole = [[False] * w for _ in range(h)]
    ...
    while next_visit:
        ...
        next_visit.extendleft(
            mark(i, j, grid, come_from_left, come_from_top, ishole, w, h))

    return grid[h-1][w-1]


def mark(i, j, grid, come_from_left, come_from_top, ishole, w, h):
    next_visit = []

    if j == w - 1 and i != h - 1 and not come_from_left[i+1][j] and not ishole[i+1][j]:
        # rightmost column
        grid[i+1][j] += grid[i][j]
        next_visit.append((i+1, j))
        come_from_left[i+1][j] = True
    elif i == h - 1 and j != w - 1 and not come_from_top[i][j+1] and not ishole[i][j+1]:
        # bottommost row
        grid[i][j+1] += grid[i][j]
        next_visit.append((i, j+1))
        come_from_top[i][j+1] = True
    else:
        if i != h - 1 and not come_from_left[i+1][j] and not ishole[i+1][j]:
            grid[i+1][j] += grid[i][j] / 2
            next_visit.append((i+1, j))
            come_from_left[i+1][j] = True
        if j != w - 1 and not come_from_top[i][j+1] and not ishole[i][j+1]:
